...
def csv_to_df_merge(_flist, fnum=None): 
    ...
    for file in _flist:
        ...
        #print(_csvdf.iloc[:,0])
        #print(_csvdf)
        #print(pd.to_datetime(_csvdf[0]))     #datetime으로 변환?
...
if __name__== "__main__" :
    ...
    date_test = input("yyyy-mm-dd hh:mm:ss")
    #unix_test = '1970-01-01 00:00:00'
    date_time = datetime.strptime(date_test, '%Y-%m-%d %H:%M:%S')                 # 입력 값 datetime으로 변환
    unix_time = datetime.strptime(date_test, '%Y-%m-%d %H:%M:%S').timestamp()     # 입력 값 unixtime으로 변환
    #unix_datetime = 

    tt = (df_m.timestamp == date_time)
    if tt:
        print("1")
    else:
        print("0")
